...
export interface HistoryOutboxEvent {
  id: string;
  workspaceId: string;
  actorId: string;
  eventType:
    "resource.revision_created" | "resource.deleted" | "resource.restored";
  subjectId: string;
  occurredAt: string;
...
export class HistoryConflictError extends Error {
  ...
  constructor() {
    super("The resource changed after the supplied base revision.");
...
export class ResourceHistoryService {
  ...
  createRevision(input: CreateRevisionInput): Promise<Revision> {
    return this.repository.transaction((repository) =>
      this.createRevisionWithin(repository, input),
    );
  }

  private async createRevisionWithin(
    repository: HistoryRepository,
    input: CreateRevisionInput,
  ): Promise<Revision> {
    const resource = await requiredResource(
      repository,
      input.workspaceId,
      input.resourceId,
    );
    const current = resource.currentRevisionId
      ? await requiredRevision(
          repository,
          input.workspaceId,
          resource.currentRevisionId,
        )
      : null;
    if (resource.currentRevisionId !== input.expectedRevisionId)
    ...
    const revision: Revision = {
      ...
      schemaPackageId: input.schemaPackageId,
      schemaVersion: input.schemaVersion,
      canonicalPayload: input.canonicalPayload,
      canonicalPayloadHash: hashCanonical(input.canonicalPayload),
    ...
    if (
      !(await repository.setResourceHead({
        workspaceId: resource.workspaceId,
        resourceId: resource.id,
  ...
  deleteResource(input: {
    ...
    return this.repository.transaction(async (repository) => {
      ...
      const tombstone: Tombstone = {
        ...
        priorRevisionId: resource.currentRevisionId,
        restoredAt: null,
      ...
      if (
        !(await repository.setResourceHead({
          ...
          resourceId: resource.id,
          expectedRevisionId: input.expectedRevisionId,
          nextRevisionId: resource.currentRevisionId,
  ...
  restoreResource(input: {
    ...
    return this.repository.transaction(async (repository) => {
      ...
      const tombstone = await repository.getOpenTombstone(
        input.workspaceId,
        input.resourceId,
      );
      if (!tombstone?.priorRevisionId)
        throw new Error("No recoverable deletion exists for this resource.");
      if (tombstone.recoverUntil && tombstone.recoverUntil < this.now())
        throw new Error("The recovery window has expired.");
      const prior = await requiredRevision(
        repository,
        input.workspaceId,
        tombstone.priorRevisionId,
      );
      const restored = await this.createRevisionWithin(repository, {
        workspaceId: input.workspaceId,
        resourceId: input.resourceId,
        expectedRevisionId: resource.currentRevisionId,
        schemaPackageId: prior.schemaPackageId,
        schemaVersion: prior.schemaVersion,
        canonicalPayload: prior.canonicalPayload,
        actorId: input.actorId,
        source: "restore",
...
async function requiredResource(
  ...
  const value = await repository.getResource(workspaceId, resourceId);
...
async function requiredRevision(
  ...
  const value = await repository.getRevision(workspaceId, revisionId);
  if (!value) throw new Error("Revision not found.");
  return value;
}
