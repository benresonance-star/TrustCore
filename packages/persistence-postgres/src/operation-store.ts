...
export class PostgresIngestOperationStore implements IngestOperationStore {
  ...
  begin(input: {
    ...
    return inTransaction(this.pool, async (db) => {
      ...
      const result = await db.query<OperationRow>(
        "INSERT INTO operations (id,workspace_id,operation_type,idempotency_key,state,requested_by,requested_by_principal_type,request_json,result_json) VALUES ($1,$2,'object.ingest',$3,'requested',$4,$5,$6::jsonb,$7::jsonb) ON CONFLICT (operation_type,workspace_id,idempotency_key,idempotency_scope) DO UPDATE SET updated_at=operations.updated_at RETURNING id,workspace_id,state,request_json,result_json",
        [
          randomUUID(),
          input.workspaceId,
          ...
          JSON.stringify({ checkpoints: [], context: {} }),
        ],
      );
  ...
  saveContext(
    ...
    return this.update(
      workspaceId,
      operationId,
      "UPDATE operations SET state=$2,result_json=jsonb_set(COALESCE(result_json,'{}'::jsonb),'{context}',$3::jsonb,true),updated_at=now() WHERE id=$1",
      [operationId, state, JSON.stringify(context)],
    );
  }
  ...
  markCheckpoint(
    ...
    return this.update(
      workspaceId,
      operationId,
      "UPDATE operations SET result_json=jsonb_set(COALESCE(result_json,'{}'::jsonb),'{checkpoints}',COALESCE(result_json->'checkpoints','[]'::jsonb)||to_jsonb($2::text),true),state=$2,updated_at=now() WHERE id=$1 AND NOT COALESCE(result_json->'checkpoints','[]'::jsonb) ? $2",
  ...
  complete(
    ...
    return this.update(
      workspaceId,
      operationId,
      "UPDATE operations SET state='completed',result_json=jsonb_set(COALESCE(result_json,'{}'::jsonb),'{result}',$2::jsonb,true),completed_at=COALESCE(completed_at,now()),updated_at=now(),error_code=NULL WHERE id=$1",
  ...
  fail(workspaceId: string, operationId: string, error: string): Promise<void> {
    return this.update(
      workspaceId,
      operationId,
      "UPDATE operations SET state=CASE WHEN state='completed' THEN state ELSE 'failed_retryable' END,error_code=CASE WHEN state='completed' THEN error_code ELSE $2 END,retry_count=CASE WHEN state='completed' THEN retry_count ELSE retry_count+1 END,updated_at=now() WHERE id=$1",
      [operationId, error.slice(0, 512)],
    );
  }
  ...
  private update(
    ...
    return inTransaction(this.pool, async (db) => {
      ...
      if (changed.rowCount !== 1 && !sql.includes("NOT COALESCE"))
        throw new Error("Ingest operation update was not applied.");
    });
  }
}
